...
class JanelaTop(ctk.CTkToplevel):
    def __init__(self, geometry='400x400', resizable=(True, True), title='Janela'):
        ...
        self.mostrar_na_frente()

    def mostrar_na_frente(self):
        self.lift()
        self.attributes('-topmost', True)
        self.focus_set()
        JanelaTop.ultima_janela = self
...
class Validacoes:
    ...
    def validar_email(self, widget, alteracao: bool = None):
        ...
        if not re.match(regex, widget.get()):
            if widget.get() and db.consulta_dado_especifico_bd(widget.get(), 'email', 'usuarios'):
    ...
    def validar_apelido(self, widget, alteracao: bool = None):
        if len(widget.get()) < 6:
            if widget.get() and db.consulta_dado_especifico_bd(widget.get(), 'apelido', 'usuarios'):
                self.janel_erro(
    ...
    def validar_cpf(self, widget, alteracao: bool = None):
        ...
        if len(list(cpf)) != 11:
            if widget.get() and db.consulta_dado_especifico_bd(widget.get(), 'cpf', 'usuarios'):
                self.janel_erro(
                    'Já existe usuário cadastrado com esse CPF', widget)
                return False
            if widget.get() and db.consulta_dado_especifico_bd(widget.get(), 'cpf', 'clientes'):
                self.janel_erro(
    ...
    def validar_cnpj(self, widget, alteracao=None):
        ...
        if len(cnpj) != 14:
            if widget.get() and db.consulta_dado_especifico_bd(widget.get(), 'cnpj', 'empresas'):
    ...
    def janel_erro(self, texto_erro, wid=None):
        ...
        if not Validacoes.texto_erro:
            ...
            if len(texto_erro) > 35:
                self.janela.geometry(f'{len(texto_erro)*10}x180')
            ...
            if texto_erro not in Validacoes.texto_erro:
                ...
                self.janela_erro_altura = (len(self.frame.winfo_children()))*90
                ...
                if len(texto_erro) > 35:
                    ...
                    self.janela.geometry(
                        f'{len(texto_erro)*10}x{self.janela_erro_altura}')
                    self.frame.pack(fill='both', expand=True)
...
class Consultas():
    ...
    def janel_consulta_usuario(self):
        ...
        def altera_usuario():
            ...
            if usuario:
                if not janela_altera_usuario_aberta and usuario:
                    ...
                    lb_cpf = ctk.CTkLabel(
                        frame, text='CPF', text_color=self.azul)
                    ...
                    bt_email_alt.grid(
                        row=6, column=1, padx=(0, 10), sticky='w')
                    ...
                    bt_apelido_alt.grid(
                        row=10, column=1, padx=(0, 10), sticky='w')
                    ...
                    janela_altera_usuario.protocol(
                        'WM_DELETE_WINDOW', fecha_janela)
        ...
        def excluir_usuario():
            ...
            lb_conf = ctk.CTkLabel(
                frame, text='Confirme a exclução do usuario:', text_color=self.azul, font=('', 20))
            lb_usuario = ctk.CTkLabel(
                frame, text=usuario[1], text_color='red', font=('', 20))
        ...
        if not Consultas.janela_consulta_usuario_aberta:
            ...
            bt_alterar = ctk.CTkButton(
                janela_consulta_usuario, text='Alterar', command=altera_usuario)
            bt_excluir = ctk.CTkButton(
                janela_consulta_usuario, text='Excluir', command=excluir_usuario)
            bt_atualizar = ctk.CTkButton(
...
class Vendas():
    ...
    def registrar_venda(self):
        ...
        if Vendas.widgets:
            ...
            val.janel_erro('Você precisa ter itens para registrar.')
        ...
        if Vendas.cpf_cliente:
            ...
            if Vendas.vl_total:
                ...
                val.janel_erro('Total nao pode ser zero')
    ...
    def emitir_nf(self):
        ...
        if Vendas.widgets:
            janela = JanelaTop('330x170', (True, True), 'Emitir NFe')
            frame = ctk.CTkFrame(janela)
            lb_titulo = ctk.CTkLabel(
            ...
            frame.pack(padx=10, pady=10, fill='both', expand=True)
            lb_titulo.grid(row=0, pady=10, column=0, columnspan=2)
            ...
            val.janel_erro('Você precisa adicionar 1 produto')
    ...
    def janel_vendas(self):
        ...
        val = Validacoes()
        val.janel_erro(
            'verificando se o erro ocorre quando eu quiser que ocorra caso isso nao ocorra ')
        ...
        if not Vendas.janela_vendas_aberta:
            ...
            self.lb_lista_compras = ctk.CTkLabel(
                self.janela_vendas, text='Lista de compras', text_color=self.azul, font=('', 20))
            self.lb_index = ctk.CTkLabel(self.janela_vendas,
                                         text=f'Codigo                           Descrição                                     Valor Unit      Quantidade       Valor                    Excluir ',
                                         text_color=self.azul)
            ...
            self.lb_sub_total_text = ctk.CTkLabel(
                self.frame_total, text=f'Subtotal R$:', text_color=self.azul, font=('', 30))
            self.lb_sub_total_valor = ctk.CTkLabel(
                self.frame_total, text=f'{0.00:.2f}', text_color=self.azul, font=('', 30), anchor='e')
            self.lb_total_pg_text = ctk.CTkLabel(
                self.frame_total, text=f'Total pago R$:', text_color=self.azul, font=('', 30))
            self.lb_total_pg_valor = ctk.CTkLabel(
                self.frame_total, text=f'{0.00:.2f}', text_color=self.azul, font=('', 30), anchor='e')
            self.lb_total_apg_text = ctk.CTkLabel(
                self.frame_total, text=f'Total a pagar R$:', text_color=self.azul, font=('', 30))
            self.lb_total_apg_valor = ctk.CTkLabel(
                self.frame_total, text=f'{0.00:.2f}', text_color=self.azul, font=('', 30), anchor='e')
            self.lb_troco_text = ctk.CTkLabel(
                self.frame_total, text=f'Troco R$:', text_color=self.azul, font=('', 30))
...
class Cadastros():
    ...
    def janel_cad_perfil(self):
        ...
        if not Cadastros.janela_cad_perfil_aberta:
            ...
            frame_perfil = ctk.CTkFrame(janela_cad_perfil)
            frame_inf = ctk.CTkFrame(frame_perfil, width=140, height=70)

            lb_titulo = ctk.CTkLabel(
                frame_perfil, text='Cadastro de Perfil', text_color=self.azul)
            et_cpf = ctk.CTkEntry(
                frame_perfil, placeholder_text='Digite seu CPF')
            op_perfil = ctk.CTkOptionMenu(
                frame_perfil, values=['Atendente'])
            lb_usuario_encontrado = ctk.CTkLabel(frame_inf, text='')

            bt_salvar = ctk.CTkButton(
                frame_perfil, text='Salvar', width=70, command=cad_perfil)

            frame_perfil.pack(padx=10, pady=10, fill='both', expand=True)
            lb_titulo.grid(row=0, padx=10, pady=5)
            et_cpf.grid(row=2, padx=10, pady=5)
            op_perfil.grid(row=3, padx=10, pady=5)
            frame_inf.grid(row=4, padx=10, pady=5)
            lb_usuario_encontrado.pack()
            bt_salvar.grid(row=6, padx=10, pady=(10, 5))
    ...
    def janel_cad_usuario(self):
        ...
        if not Cadastros.janela_cad_usuario_aberta:
            ...
            cad_janela = JanelaTop('330x600', (False, False), 'Cadastro')

            frame = ctk.CTkFrame(master=cad_janela, width=310, height=600)
            frame.pack(padx=10, pady=10, fill='both', expand=True)

            titulo_cad = ctk.CTkLabel(
                master=frame, text='Cadastro de usuário', text_color=self.azul, font=('', 25))
            lb_nome = ctk.CTkLabel(
    ...
    def janel_cad_empresa(self):
        ...
        if not Cadastros.janela_cad_empresa_aberta:
            ...
            janela_cad_empresa = JanelaTop(
                '340x800', (False, False), 'cadastro de empresas')

            frame = ctk.CTkFrame(janela_cad_empresa)

            lb_titulo = ctk.CTkLabel(
                frame, text='Cadastro empresas', text_color=self.azul, font=('', 20))
            lb_razao_social = ctk.CTkLabel(
                frame, text='Razão social', text_color=self.azul, font=('', 12), anchor='s')
            razao_social = ctk.CTkEntry(
                frame, placeholder_text='Razão social', width=300)
            lb_nome_fantasia = ctk.CTkLabel(
                frame, text='Nome fantasia', text_color=self.azul, font=('', 12), anchor='s')
            nome_fantasia = ctk.CTkEntry(
                frame, placeholder_text='Nome fantasia')
            lb_cnpj = ctk.CTkLabel(
                frame, text='CNPJ', text_color=self.azul, font=('', 12), anchor='s')
            cnpj = ctk.CTkEntry(frame, placeholder_text='CNPJ')
            lb_cod_regime_trib = ctk.CTkLabel(
                frame, text='Cod regime trib', text_color=self.azul, font=('', 12), anchor='s')
            cod_regime_trib = ctk.CTkOptionMenu(
                frame, values=['Simples nacional', 'Lucro presumido', 'Lucro real'])
            lb_insc_estadual = ctk.CTkLabel(
                frame, text='Insc estadual', text_color=self.azul, font=('', 12), anchor='s')
            insc_estadual = ctk.CTkEntry(
                frame, placeholder_text='Inscrição estadual')
            lb_insc_municipal = ctk.CTkLabel(
                frame, text='Insc municipal', text_color=self.azul, font=('', 12), anchor='s')
            insc_municipal = ctk.CTkEntry(
                frame, placeholder_text='Inscrição municipal')
            lb_cnae = ctk.CTkLabel(
                frame, text='Cod CNAE', text_color=self.azul, font=('', 12), anchor='s')
            cod_cnae = ctk.CTkEntry(frame, placeholder_text='Código CNAE')
            lb_logradouro = ctk.CTkLabel(
                frame, text='Logradouro', text_color=self.azul, font=('', 12), anchor='s')
            end_logradouro = ctk.CTkEntry(
                frame, placeholder_text='Logradouro', width=300)
            lb_numero = ctk.CTkLabel(
                frame, text='Nº', text_color=self.azul, font=('', 12), anchor='s')
            end_numero = ctk.CTkEntry(
                frame, placeholder_text='Nº', width=50)
            lb_bairro = ctk.CTkLabel(
                frame, text='Bairro', text_color=self.azul, font=('', 12), anchor='s')
            end_bairro = ctk.CTkEntry(
                frame, placeholder_text='Bairro', width=200)
            lb_municipio = ctk.CTkLabel(
                frame, text='Municipio', text_color=self.azul, font=('', 12), anchor='s')
            end_municipio = ctk.CTkEntry(
                frame, placeholder_text='Município')
            lb_uf = ctk.CTkLabel(
                frame, text='UF', text_color=self.azul, font=('', 12), anchor='s')
            end_uf = ctk.CTkEntry(frame,
                                  placeholder_text='UF', width=50)
            lb_cep = ctk.CTkLabel(
                frame, text='CEP', text_color=self.azul, font=('', 12), anchor='s')
            end_cep = ctk.CTkEntry(frame, placeholder_text='CEP')
            lb_funcao = ctk.CTkLabel(
                frame, text='Função', text_color=self.azul, font=('', 12), anchor='s')
            ...
            lb_titulo.grid(row=0, column=0, columnspan=2, pady=10, padx=10)
            lb_razao_social.grid(row=1, padx=10, sticky='w')
            razao_social.grid(row=2, column=0, columnspan=2,
                              padx=10, sticky='w')
            lb_nome_fantasia.grid(row=3, padx=10, sticky='w')
            nome_fantasia.grid(row=4, column=0, padx=10, sticky='w')
            lb_cnpj.grid(row=5, padx=10, sticky='w')
            cnpj.grid(row=6, column=0,  padx=10, sticky='w')
            lb_cod_regime_trib.grid(row=7, padx=10, sticky='w')
            cod_regime_trib.grid(row=8, column=0,  padx=10, sticky='w')
            lb_insc_estadual.grid(row=9, padx=10, sticky='w')
            insc_estadual.grid(row=10, column=0,  padx=10, sticky='w')
            lb_insc_municipal.grid(row=11, padx=10, sticky='w')
            insc_municipal.grid(row=12, column=0,  padx=10, sticky='w')
            lb_cnae.grid(row=13, padx=10, sticky='w')
            cod_cnae.grid(row=14, column=0,  padx=10, sticky='w')
            lb_logradouro.grid(row=15, padx=10, sticky='w')
            end_logradouro.grid(row=16, column=0, columnspan=2,
                                padx=10, sticky='w')
            lb_numero.grid(row=17, column=1, padx=10, sticky='w')
            end_numero.grid(row=18, column=1,  padx=10, sticky='w')
            lb_bairro.grid(row=17, padx=10, sticky='w')
            end_bairro.grid(row=18, column=0,  padx=10, sticky='w')
            lb_municipio.grid(row=21, padx=10, sticky='w')
            end_municipio.grid(row=22, column=0,  padx=10, sticky='w')
            lb_uf.grid(row=21, column=1, padx=10, sticky='w')
            end_uf.grid(row=22, column=1,  padx=10, sticky='w')
            lb_cep.grid(row=25, padx=10, sticky='w')
            end_cep.grid(row=26, column=0,  padx=10, sticky='w')
            lb_funcao.grid(row=27, padx=10, sticky='w')
            funcao.grid(row=28, column=0,  padx=10, sticky='w')
            bt_salvar.grid(row=29, column=0, columnspan=2, pady=20, padx=10)
